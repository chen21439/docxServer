        ...
        if (typeof axios !== 'undefined') {
          ...
          const apiRequestData = {
            ...
            messages: [
              {
                role: "system",
                content: selectedText ? `请基于以下文本内容回答用户问题：\n${selectedText}` : "你是一个AI助手，请回答用户的问题。"
              },
              {
                role: "user",
                content: message
          ...
          axios.post('http://112.111.20.89:8888/v1/chat/completions', apiRequestData, {
          ...
          .then(res => {
            ...
            // 提取AI回复内容
            const aiResponse = res.data?.choices?.[0]?.message?.content || res.data?.message || '暂无回复';
          ...
          .catch(err => {
            ...
            const errorMsg = err.response?.data?.message || err.message || '请求失败，请稍后再试';
